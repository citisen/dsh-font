  ...
  ctx.inject(['settings'], (settingsCtx) => {
    if (typeof settingsCtx.settings.register !== 'function') {
      settingsCtx.logger.error(
        `dsh-font: this dsh has no settings.register() (0.1.7-alpha.1 replaced the settings API), ` +
          `so the durable "${FONT_SETTINGS_NAMESPACE}" section is not registered and font choices ` +
          'cannot be saved. Pin dsh to 0.1.5-rc.x (latest/next), or upgrade @citisen/dsh-font.',
      )
      return
    }
