 ...
 * that the control they just used is not going to save anything.
 */
function reportMissingSettings() {
  if (reportedMissingSettings) return
  reportedMissingSettings = true
  console.error(
    `${PLUGIN_ID}: dsh is not providing the "${SETTINGS_SERVICE}" service, so the Fonts row shows its ` +
      `shipped defaults and changes are not saved. dsh 0.1.7-alpha.1 replaced it with ` +
      `"${REPLACEMENT_SETTINGS_SERVICE}"; this build targets the dsh 0.1.5-rc.x line (latest/next). ` +
      'Pin dsh to 0.1.5-rc.x, or install a newer @citisen/dsh-font.',
  )
}
 ...
 * "waiting for service". The section is bound optionally in `apply` instead, so
 * a composition this build does not recognize still gets the row, the shipped
 * defaults, and a message that names the reason.
 *
...
function apply(ctx) {
  ...
  // Bind the durable section when dsh provides the settings service, and follow
  // it while it stays: a replacement or an unload puts the row back on defaults.
  ctx.inject([SETTINGS_SERVICE], (settingsCtx) => {
    ...
    scope = bound
    settingsCtx.effect(() => bound.subscribe(adopt), 'dsh-font: settings adoption')
    adopt()
    return () => {
