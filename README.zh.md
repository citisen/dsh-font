...
上被报成「未激活」的条目；`0.2.3` 两条线的设置都会说，`0.2.4` 起还不再借用设计系统的图标。

最后这条值得单独说清楚，因为它正是移植设置时最容易藏住的故障。设计系统的图标导出按笔画粗细命名，
而名字在两条 dsh 线之间改过（0.1.5-rc.x 是 `IconChevronDownOutline14`，0.1.7 是
`IconChevronDownOutlineRegular`）。名字没了的图标不会渲染成"缺一张图"，而是以 `undefined` 顶替
组件类型，于是 React 在渲染这一行时抛 "Element type is invalid"，**整个字体行直接从设置页面上
消失**，只留一条控制台报错。现在这一行的箭头是插件自己画的 —— 也正因如此，它的客户端 bundle 只向
shell 要 `react` 和 `@deepseek-ai/dsh-client-store`。

### 0.1.7 改名时丢掉的设置

dsh 0.1.7 会把旧的 `$DSH_HOME/settings.yaml` 导入一次 —— 每个 section 写进同名条目 —— 并把文件
改名为 `settings.yaml.imported`。在 `0.2.4` 之前，本插件的条目叫 `font`，于是
